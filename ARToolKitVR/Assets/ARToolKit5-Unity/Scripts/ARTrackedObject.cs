			...
			if (origin == null) {
				...
				if (marker == null) {
					...
					if (baseMarker != null && marker.Visible) {
                        ...
                        //transform.localRotation = Quaternion.Inverse(ARUtilityFunctions.QuaternionFromMatrix(pose));

                        Quaternion quat = ARUtilityFunctions.QuaternionFromMatrix(pose);

                        //Vector3 myEulerAngles = quat.eulerAngles;
