...
//    	AR_MATRIX_CODE_GLOBAL_ID = 2830
        ...
        if (!_sceneConfiguredForVideo) {
            ...
            if (!PluginFunctions.arwIsRunning()) {
				...
				if (!VideoIsStereo) {
					...
					CreateVideoBackgroundMesh(0, _videoWidth0, _videoHeight0, BackgroundLayer0, out _videoColorArray0, out _videoColor32Array0, out _videoTexture0, out _videoMaterial0);
					CreateVideoBackgroundMesh(1, _videoWidth1, _videoHeight1, BackgroundLayer1, out _videoColorArray1, out _videoColor32Array1, out _videoTexture1, out _videoMaterial1);
	...
	private GameObject CreateVideoBackgroundMesh(int index, int w, int h, int layer, out Color[] vbca, out Color32[] vbc32a, out Texture2D vbt, out Material vbm)
	{
		...
		GameObject vbmgo = myCamera.gameObject;
		if (vbmgo == null) {
        ...
        webcamPlane.transform.localScale = new Vector3(webcamPlane.transform.localScale.x * w/h, webcamPlane.transform.localScale.y, webcamPlane.transform.localScale.z);
        
		MeshFilter filter = webcamPlane.GetComponent<MeshFilter>();
        ...
        webcamPlane.GetComponent<Renderer>().material = vbm;
		
        //GUITexture backgroundTexture = vbmgo.GetComponent<GUITexture>();
        //backgroundTexture.texture = vbm.mainTexture;
        //backgroundTexture.transform.Rotate(new Vector3(0, 90, 0));
        return vbmgo;
	}
